...
- **桌面级时间线** — `Timeline` 三种对齐（轴在左 / 轴在右 / 左右交替），时间戳可放
  对侧独立成列（默认，VS Code 时间线那种形态）/ 标题上方 / 同行 / 隐藏；
  节点有语义色、三种形态（实心 / 空心环 / 轻量点）与三档尺寸，`done` 自动变对勾、
  `pending` 在末尾挂一个"进行中"空心环；整行可点，悬停高亮与点击水波纹由
  Material 的 `ink` 绘制（不需要 Python 侧重绘，大列表也不掉帧）
